...
void fbdev_flush(lv_disp_drv_t * drv, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    int32_t act_x2 = area->x2 > (int32_t)vinfo.yres - 1 ? (int32_t)vinfo.yres - 1 : area->x2;
    int32_t act_y2 = area->y2 > (int32_t)vinfo.xres - 1 ? (int32_t)vinfo.xres - 1 : area->y2;
	...
	#if 1
		int32_t x = 0;
		long int color_idx = 0;
        lv_coord_t h = (act_y2 - act_y1 + 1);
        uint32_t color_cnt = w * h;

        location = (act_x1 + vinfo.yoffset) * vinfo.xres + vinfo.xres - act_y1 + vinfo.xoffset;
        for(x = color_cnt - w; x < color_cnt; x++) {
            color_idx = x;
            for(y = location - h; y < location; y++) {
                fbp32[y] = color_p[color_idx].full;
                color_idx = color_idx - w;
            }
            location = location + vinfo.xres;
        }
