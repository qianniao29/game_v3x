...
extern u32 *nes_fbp; //LTDC LCD帧缓存数组指针,必须指向对应大小的内存区域 
...
void scanline_draw(int LineNo)
{
  ...
  u32 temp;
  u32 color=0;
  u32 *plcd= nes_fbp;  //指向显存地址减少数组访问,提高速度
  u32 offest = LCD_HEIGHT * nes_xoff + (LCD_HEIGHT - 1) - (LineNo + nes_yoff) * 2;
  ...
  for(i=sx;i<ex;i++)
  {
  	//1个点扩大为4个点，适应480*800的屏幕
    plcd[offest]=NES_Palette[ppu->dummy_buffer[i]];
	plcd[offest-1]=NES_Palette[ppu->dummy_buffer[i]];
	offest += LCD_HEIGHT;
	plcd[offest]=NES_Palette[ppu->dummy_buffer[i]];
	plcd[offest-1]=NES_Palette[ppu->dummy_buffer[i]];
	offest += LCD_HEIGHT;
